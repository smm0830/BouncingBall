...
/*
 * Shane Malinowski
 * https://smm0830.github.io/BouncingBall/ 
 *
 * The three meaningful changes I made are as follows:
 *
 * 1.) I changed the music file to a public domain country song I found on the internet
 * which is meaningful to me as I am from a more rural area.
 *
 * 2.) I changed the textbox to be a counter to count every time a ball hits the box. I
 * did this by adding a global variable that increments every time a ball reverses after
 * hitting the 'hitbox' in the checkForHitBox method.
 *
 * 3.) I changed the variable in which the balls spawn so they no longer get stuck within
 * the textbox, however they still sometimes get stuck in the wall.
 */

const BOX_WIDTH  = 275;  // textbox dimensions
...
function preload() {
  ...
  sound = loadSound("Alabama-Bound-by-Roger-McGuinn.mp3");  // preload the sound file
}
...
function createBox() {
  ...
  textAlign(CENTER);
  text('Text Box Hits: ' + y, BOX_WIDTH/2, BOX_HEIGHT/2);   
...
class Ball { // Constructor
  ...
  constructor() {
    // initial position
    this.ballX = random(150, width)
    this.ballY = random(150, height)
